...
class VisaPassengerSelectVC: BaseTableVC {
    ...
    static var newInstance: VisaPassengerSelectVC? {
        let storyboard = UIStoryboard(name: Storyboard.Visa.name,
                                      bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: self.className()) as? VisaPassengerSelectVC
        return vc
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        adultsCount = Int(defaults.string(forKey: UserDefaultsKeys.visaadultCount) ?? "1") ?? 0
        childCount = Int(defaults.string(forKey: UserDefaultsKeys.visachildCount) ?? "0") ?? 0
        infantsCount = Int(defaults.string(forKey: UserDefaultsKeys.visainfantsCount) ?? "0") ?? 0
    ...
    func gotoBookFlightVC() {
        ...
        defaults.set(totaltraverlers, forKey: UserDefaultsKeys.visatotalpassengercount)
